...
suite('Donation API tests', function () {
  ...
  test('create a donation', function () {
    ...
    assert(_.some([returnedDonations[0]], donations[0]), 'returned donation must be superset of donation');
  }).timeout(15000);
  ...
  test('create multiple donations', function () {
    ...
    for (var i = 0; i < donations.length; i++) {
      assert(_.some([returnedDonations[i]], donations[i]), 'returned donation must be superset of donation');
    }
  }).timeout(15000);

  test('delete all donations for a candidate', function () {
    const returnedCandidate = donationService.createCandidate(newCandidate);
    for(var i = 0; i < donations.length; i++) {
      donationService.makeDonation(returnedCandidate._id, donations[i]);
    }
